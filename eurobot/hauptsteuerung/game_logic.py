...
class Task():
    def __init__(self, robots, can_socket, can_id):
        self.robots = robots
        self.can_command_queue = queue.Queue()
        can_socket.create_queue(can_id+1, self.can_command_queue)
        self.can_status_queue = queue.Queue()
        can_socket.create_queue(can_id+1, self.can_status_queue)
